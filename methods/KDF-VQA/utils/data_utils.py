...
def load_sparse_adj_data_with_contextnode(adj_pk_path, max_node_num, num_choice):
    cache_path = adj_pk_path +'qagnn_sem.loaded_cache'
    use_cache = True
    # print(max_node_num)
    ...
    # print("concept_ids:", concept_ids.size())
...
def load_bert_xlnet_roberta_input_tensors(statement_jsonl_path, model_type, model_name, max_seq_length):
    ...
    def _truncate_seq_pair(tokens_a, tokens_b, max_length):
        ...
        # 一个简单的启发式方法，它总是一次截断较长的序列中的一个符号。这比从每个序列中截断相同比例的标记更有意义，因为如果一个序列非常短，那么被截断的每个标记 都可能包含比长序列更多的信息
        while True:
            total_length = len(tokens_a) + len(tokens_b)
            if total_length <= max_length:
                break
