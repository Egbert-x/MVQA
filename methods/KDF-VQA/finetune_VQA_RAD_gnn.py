...
from dataset.slake_dataset_gnn import VQARAD_VQA_Dataset
...
class VQATrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):  ## compute loss这个步骤实际上定义了 forward和loss的计算过程。。。
        ...
        # generated_texts = get_generated_texts(label, outputs.argmax(-1), model.tokenizer)
...
class ModelArguments:
    ...
    pre_trained: Optional[str] = field(default="./Results/VQA_lora_pmcclip/vqa/checkpoint-6500")
...
class DataArguments:
    ...
    img_root_dir: str = field(default='../../VQA-RAD/VQA_RAD Image Folder/', metadata={"help": "Path to the training data."})
    Train_csv_path: str = field(default='../../VQA-RAD/data/train/train.csv', metadata={"help": "Path to the training data."})
    Test_csv_path: str = field(default='../../VQA-RAD/data/test/test.csv',metadata={"help": "Path to the training data."})
    Train_adj_path: str = field(default='../../VQA-RAD/data/graph/train_sem.graph.adj.pk', metadata={"help": "Path to the training data."})
    Test_adj_path: str = field(default='../../VQA-RAD/data/graph/test_sem.graph.adj.pk', metadata={"help": "Path to the training data."})
...
def main():
    ...
    Train_dataset = VQARAD_VQA_Dataset(data_args.Train_csv_path, data_args.Train_adj_path, data_args.img_root_dir, data_args.image_res,is_train=True)
    Eval_dataset = VQARAD_VQA_Dataset(data_args.Test_csv_path, data_args.Test_adj_path, data_args.img_root_dir, data_args.image_res, is_train=True)

    print("Setup Model")
    ckp = model_args.pre_trained + '/pytorch_model.bin'
