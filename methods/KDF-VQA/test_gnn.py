...
def main():
    ...
    print("Setup Data")
    Test_dataset = Binary_VQA_Dataset(data_args.Test_csv_path, data_args.Test_adj_path, data_args.img_root_dir, data_args.image_res, is_blank=True, is_train=False)
    ...
    with open(os.path.join(training_args.output_dir, 'result.csv'), mode='w') as outfile:
        ...
        for sample in tqdm.tqdm(Test_dataloader):
            ...
            index_pred = find_most_similar_index(Choice_list, generated_texts)
            index_label = find_most_similar_index(Choice_list, Answer)
            corret = 0
            if index_pred == index_label:
                ACC = ACC + 1
                corret = 1
            writer.writerow([img_path, Answer, generated_texts, corret])
            cc = cc + 1
        print(ACC / cc)
