...
class DataArguments:
    ...
    Test_csv_path: str = field(default='../../Slake1.0_new/validate.csv',metadata={"help": "Path to the training data."})
    Train_adj_path: str = field(default='../../Slake1.0_new/graph/train_sem.graph.adj.pk', metadata={"help": "Path to the training data."})
    Test_adj_path: str = field(default='../../Slake1.0_new/graph/validate_sem.graph.adj.pk',metadata={"help": "Path to the training data."})
...
def main():
    ...
    Train_dataset = SLAKE_VQA_Dataset(data_args.Train_csv_path, data_args.Train_adj_path, data_args.img_root_dir, data_args.image_res,is_train=True)
    Eval_dataset = SLAKE_VQA_Dataset(data_args.Test_csv_path, data_args.Test_adj_path, data_args.img_root_dir, data_args.image_res,is_train=True)
