...
import transformers
from Dataset.PMC_QA_Dataset_triples import PMC_QA_Dataset_triples
...
class ModelArguments:
    ...
    visual_model_path: Optional[str] = field(default='../../../PMC-VQA/src/MedVInT_TE/models/pmc-clip/checkpoint.pt')
...
@dataclass
class DataArguments:
    img_dir: str = field(default='../../../PMC-VQA/PMC-VQA/images/', metadata={"help": "Path to the training data."})
    Test_csv_path: str = field(default='../../../PMC-VQA/PMC-VQA/test.csv', metadata={"help": "Path to the training data."})
    Test_triples_path: str = field(default='../../../PMC-VQA/PMC-VQA/triples/test_sem.triples.json', metadata={"help": "Path to the training data."})
...
def main():
    ...
    Test_dataset = PMC_QA_Dataset_triples(data_args.img_dir, data_args.Test_csv_path, data_args.Test_triples_path, data_args.tokenizer_path, mode='Test', start=row_count)
    ...
    with open(os.path.join(training_args.output_dir, 'result_response.csv'), mode='w') as outfile:
        writer = csv.writer(outfile)
        writer.writerow(['Figure_path', 'Pred', 'Label', 'Correct'])
        for sample in tqdm.tqdm(Test_dataloader):
            input_ids = Test_dataset.tokenizer(sample['input_ids'], return_tensors="pt").to(device)
