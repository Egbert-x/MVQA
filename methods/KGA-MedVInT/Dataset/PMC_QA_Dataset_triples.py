...
class PMC_QA_Dataset_triples(Dataset):
    def __init__(self, img_dir, csv_path, triples_path, tokenizer_path, img_tokens=32, seq_length=512, voc_size=32000,
                 mode='Train', start=0, text_type='random', no_image=False):
        self.img_root = img_dir
        self.data = pd.read_csv(csv_path).iloc[start:]

        f = open(triples_path, 'r')
        content = f.read()
        self.triples = json.loads(content)
    ...
    def random_answer(self, Prompt, Question, choice_list, Answer, caption):
        p = random.random()
        Combined_choice = ''
        ...
        if self.text_type == 'random':
            if p <= 0.33:
                ...
                pre_text = 'Prompt: ' + Prompt + 'Question: ' + Question + 'The Answer is:'
                final_o = 'Prompt: ' + Prompt + 'Question: ' + Question + 'The Answer is:' + Answer
            if p > 0.33 and p <= 0.66:
                pre_text = 'Prompt: ' + Prompt + 'Question: ' + Question + 'Choices:' + Combined_choice + 'The Answer is:'
                final_o = 'Prompt: ' + Prompt + 'Question: ' + Question + 'Choices:' + Combined_choice + 'The Answer is:' + Answer
            if p > 0.66:
                pre_text = ''
                final_o = caption
    ...
    def __getitem__(self, idx):
        ...
        Anwser = sample['Answer_label']

        pre_p = 'Below are the facts that might be relevant to answer the question: '
        triples = self.triples[idx]['triples']
        # print(triples)
        for i in range(len(triples)):
            if i > 4:
                break
            triple = '(' + triples[i][0] + ',' + triples[i][1] + ',' + triples[i][2] + ')' + ','
            pre_p += triple
        Prompt = pre_p[:-1]
        ...
        if self.mode == 'Train':
            pre_text, final_o = self.random_answer(Prompt, Question, choice_list, Anwser, caption)
        ...
        if self.mode == 'Test':
            ...
            if not self.no_image:
                item = {
                    'input_ids': 'Prompt: ' + Prompt + 'Question: ' + Question + 'Choices:' + Combined_choice + 'The Answer is:',
            ...
            if not self.no_image:
                if self.text_type == 'blank':
                    item = {
                        'input_ids': 'Prompt: ' + Prompt + 'Question: ' + Question + 'The Answer is:',
                        'img_path': sample['Figure_path'],
                        'images': image,
                        'labels': Anwser,
                        'Choice_A': Choice_A,
